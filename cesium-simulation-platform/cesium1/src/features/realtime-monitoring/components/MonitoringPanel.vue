    ...
    <div class="flex flex-col gap-4">
      ...
      >
        ...
        <TimelinePlayer
          ...
          :initial-time="currentTimestamp"
          :data-source-mode="connectionType === 'simulated' ? 'simulated' : 'external'"
          @time-change="handleTimelineTimeChange"
          @play-state-change="handleTimelinePlayStateChange"
      ...
      <RoadDrawingTool
        :viewer="viewerRef"
        @path-applied="handlePathApplied"
        @default-route-set="handleDefaultRouteSet"
      />

      <!-- 矿卡实时信息 -->
      <TruckInfoPanel
        :truck-states="truckStates"
      ...
      >
        ...
        <div
          class="px-3 py-2.5 rounded bg-blue-500/10 border border-blue-500/20 text-xs text-gray-300 mb-3"
        >
        ...
        <div class="space-y-2">
          ...
          <div v-if="healthAlerts.length > 0" class="space-y-2">
            <div
              v-for="alert in healthAlerts.slice(0, 3)"
              :key="alert.key || `${alert.truckId}-${alert.type}`"
        ...
        <div class="flex items-center justify-between gap-3">
          ...
          >
            <span
              ...
              :class="isMonitoring ? 'bg-green-400 animate-pulse' : 'bg-gray-400'"
            ></span>
      ...
      >
        ...
        <div v-if="showDataSourceConfig" class="space-y-2">
          <div class="grid grid-cols-3 gap-2">
            <button
              class="px-3 py-2 rounded text-xs font-medium transition-all border"
              :class="
                connectionType === 'simulated'
                  ? 'bg-green-500/20 text-green-400 border-green-500/30'
                  : 'bg-white/5 text-gray-300 border-white/10 hover:bg-white/10'
              "
              @click="handleSwitchDataSource('simulated')"
            >
              🖥️ 模拟
            </button>
            <button
          ...
          <div
            class="px-3 py-2 bg-blue-500/10 border border-blue-500/20 rounded text-[10px] text-gray-400"
          >
            💡 实时数据接口需返回如下格式：
...
const httpUrl = ref('http://localhost:8080/api/trucks/positions')
...
function handlePathApplied(pathData) {
  console.log('[MonitoringPanel] 应用自定义路径:', pathData.name)
  // 设置自定义路径，矿卡将沿此路径移动
  setCustomPath(pathData.points)
}

function handleDefaultRouteSet(routeData) {
  console.log('[MonitoringPanel] 设置默认路线:', routeData.name)
