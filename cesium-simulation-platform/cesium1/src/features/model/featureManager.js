...
export class FeatureManager {
   ...
   */
  destroy() {
    if (this.tileset && this.tileLoadListener) {
      this.tileset.tileLoad.removeEventListener(this.tileLoadListener)
      this.tileLoadListener = null
    }
    this.featureMap.clear()
    this.modelList = []
  ...
  updateFeatureId(oldId, newId) {
    if (!oldId || !newId || oldId === newId) return
    const feature = this.featureMap.get(oldId)
    if (feature) {
      this.featureMap.delete(oldId)
      this.featureMap.set(newId, feature)
      try {
        if (typeof feature.setProperty === 'function') {
          feature.setProperty('id', newId)
        }
      } catch (e) {
        void e
      }
