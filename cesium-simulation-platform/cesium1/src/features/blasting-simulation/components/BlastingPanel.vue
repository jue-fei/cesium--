  ...
  <div class="h-full min-h-0 p-4 text-text-primary text-base overflow-x-hidden overflow-y-auto">
    <div class="blasting-panel">
      ...
      <div v-if="dataset" class="event-banner">
        ...
        <button class="exit-btn" title="退出爆破板块" @click="exitBlasting">退出</button>
      </div>

      <!-- 数据库事件选择 -->
      <div class="panel-section">
        <div class="panel-section-title">爆破事件</div>
...
import ConsistencyDiagnostics from './ConsistencyDiagnostics.vue'
import HistoryCompare from './HistoryCompare.vue'
import FragmentDistribution from './FragmentDistribution.vue'
...
const tabs = [
  ...
  { key: 'distribution', label: '块度分布' },
  { key: 'diagnostics', label: '一致性诊断' },
