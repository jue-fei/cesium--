<template>
  <div>
    ...
    <div class="section">
      ...
      <div class="grid-auto">
        ...
        <label class="field">
          ...
          <el-input-number v-model="sectionForm.width" :min="4" :max="30" :step="0.5" :precision="1" :controls="false" size="small" />
        </label>
        <label class="field">
          <span class="field-label">直墙高 (m)</span>
          <el-input-number v-model="sectionForm.wallHeight" :min="2" :max="15" :step="0.5" :precision="1" :controls="false" size="small" />
        </label>
        <label class="field">
          <span class="field-label">拱半径 (m)</span>
          <el-input-number v-model="sectionForm.archRadius" :min="2" :max="15" :step="0.5" :precision="1" :controls="false" size="small" />
        </label>
        <label class="field">
      ...
      <div class="row mt-1">
        <span class="hint">总高 {{ sectionDerived.totalH.toFixed(1) }}m · 面积 {{ sectionDerived.area.toFixed(2) }}m²</span>
        <button class="btn primary" style="margin-left:auto" @click="$emit('apply-section')">应用断面并重布孔</button>
      </div>
    </div>
    ...
    <div class="section">
      ...
      <div class="row">
        <select :value="selectedPresetKey" class="sel" style="flex:1;min-width:140px" @change="onPresetKeyChange">
          <option value="">-- 选择场地预设 --</option>
          <option v-for="(preset, key) in sitePresets" :key="key" :value="key">{{ preset.label }}</option>
        </select>
        <button class="btn primary" :disabled="!selectedPresetKey" @click="$emit('apply-preset')">应用</button>
      </div>
    </div>
    ...
    <div class="section">
      ...
      <div class="grid-auto">
        ...
        <label class="field">
          ...
          <el-input-number v-model="kcoParams.q" :min="0.1" :max="5" :step="0.05" :precision="3" :controls="false" size="small" />
        </label>
        <label class="field">
          <span class="field-label">B 抵抗线 (m)</span>
          <el-input-number v-model="kcoParams.B" :min="0.3" :max="5" :step="0.1" :precision="2" :controls="false" size="small" />
        </label>
        <label class="field">
          <span class="field-label">S 孔间距 (m)</span>
          <el-input-number v-model="kcoParams.S" :min="0.3" :max="6" :step="0.1" :precision="2" :controls="false" size="small" />
        </label>
        <label class="field">
          <span class="field-label">炸药类型</span>
          <select :value="explosiveType" class="sel" @change="onExplosiveSelect">
            <option v-for="(explosive, key) in explosiveTypes" :key="key" :value="key">{{ explosive.label }}</option>
          </select>
        </label>
        <label class="field">
        ...
        <label class="field">
          ...
          <el-input-number v-model="kcoParams.H" :min="1" :max="15" :step="0.1" :precision="2" :controls="false" size="small" />
        </label>
      </div>
    </div>
    ...
    <div class="section">
      ...
      <div class="grid-auto">
        <label class="field">
          ...
          <el-input-number v-model="kcoParams.xmax" :min="0.2" :max="5" :step="0.1" :precision="2" :controls="false" size="small" />
        </label>
        <label class="field">
          <span class="field-label">b 弯曲参数</span>
          <el-input-number v-model="kcoParams.b" :min="1.0" :max="5.0" :step="0.1" :precision="2" :controls="false" size="small" />
        </label>
      </div>
      <div class="row mt-2">
      ...
      <div v-show="advancedOpen" class="grid-auto mt-2">
        <label class="field">
          ...
          <el-input-number v-model="kcoParams.eta" :min="0.05" :max="0.4" :step="0.01" :precision="3" :controls="false" size="small" />
        </label>
        <label class="field">
          <span class="field-label" title="1.0=纯物理量级（默认）；调小收缩抛距、爆堆更贴掌子面">抛掷速度收缩</span>
          <el-input-number v-model="kcoParams.velocityScale" :min="0.05" :max="3" :step="0.05" :precision="2" :controls="false" size="small" />
        </label>
        <label class="field">
          <span class="field-label">W_abs 钻孔偏差 (m)</span>
          <el-input-number v-model="kcoParams.drillDeviation" :min="0" :max="0.5" :step="0.01" :precision="3" :controls="false" size="small" />
        </label>
        <label class="field">
          <span class="field-label">碎片渲染上限</span>
          <el-input-number v-model="kcoParams.fragmentCountRenderLimit" :min="40" :max="20000" :step="100" :controls="false" size="small" />
        </label>
        <label class="field">
        ...
        <label class="field">
          ...
          <el-switch :model-value="kcoParams.usePerssonVelocity !== false" @update:model-value="kcoParams.usePerssonVelocity = $event" />
        </label>
      </div>
    </div>
  </div>
...
const emit = defineEmits([
  'apply-section', 'update:cut-pattern', 'update:selected-preset-key',
  'apply-preset', 'explosive-select', 'toggle-advanced'
])

function onCutPatternChange(event) { emit('update:cut-pattern', event?.target?.value || 'diamond') }
function onPresetKeyChange(event) { emit('update:selected-preset-key', event?.target?.value || '') }
function onExplosiveSelect(event) { emit('explosive-select', event) }
</script>