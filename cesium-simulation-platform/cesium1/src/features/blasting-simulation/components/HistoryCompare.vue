<template>
  <div>
    ...
    <div class="section">
      ...
      <div class="row">
        <button class="btn" :disabled="!currentEventId" @click="addCurrentEvent">
          加入当前事件
        </button>
        <button
          class="btn primary"
          :disabled="compareEventIds.length < 2 || comparing"
          @click="$emit('compare')"
        >
          {{ comparing ? '对比中…' : '生成对比' }}
        </button>
    ...
    <template v-if="comparisonData.length">
      <div v-if="summaryRows.length" class="section">
        ...
        <div class="snapshot-list">
          <div v-for="row in summaryRows" :key="row.title" class="snapshot-row" :class="row.tone">
            ...
            <span class="snapshot-val" :class="row.tone"
              >{{ formatNum(row.value) }}{{ row.unit }}</span
            >
      ...
      <div v-if="comparisonCharts.length" class="section">
        ...
        >
          <div class="row">
            <span class="hint" style="font-weight: 600; color: var(--text-primary)">{{
              chart.label
            }}</span>
            <span class="hint" style="margin-left: auto"
              >最大 {{ formatNum(chart.max) }}{{ chart.unit }}</span
            >
          </div>
          <div class="cmp-list mt-1">
            <div v-for="(item, index) in chart.items" :key="item.name" class="cmp-row">
              ...
              <div class="cmp-track">
                <div class="cmp-fill" :style="{ width: item.percent + '%' }"></div>
              </div>
              <span class="hint" style="font-family: 'Consolas', monospace"
                >{{ formatNum(item.value) }}{{ chart.unit }}</span
              >
            </div>
          </div>
        </div>
      ...
      <div v-if="tableRows.length" class="section">
        ...
        <div class="cmp-table">
          <div class="cmp-table-head">
            <span>方案</span><span>x50</span><span>x80</span><span>抛距</span><span>振动峰值</span
            ><span>漏斗</span><span>评价</span>
          </div>
          <div v-for="row in tableRows" :key="row.name" class="cmp-table-row">
            ...
            >
            <span class="hint" style="font-family: 'Consolas', monospace"
              >{{ formatNum(row.fragmentX50) }}m</span
            >
            <span class="hint" style="font-family: 'Consolas', monospace"
              >{{ formatNum(row.fragmentX80) }}m</span
            >
            <span class="hint" style="font-family: 'Consolas', monospace"
              >{{ formatNum(row.throwDistanceMax) }}m</span
            >
            <span class="hint" style="font-family: 'Consolas', monospace"
              >{{ formatNum(row.vibrationPeak) }}cm/s</span
            >
            <span class="hint" style="font-family: 'Consolas', monospace"
              >{{ formatNum(row.craterDepth) }}×{{ formatNum(row.craterRadius) }}m</span
            >
    ...
    <div v-else-if="!comparing" class="section">
      <div class="hint">
        还没有生成对比结果。建议先把当前事件加入，再选 1-2 个历史事件一起比较。
      </div>
    </div>
  </div>
</template>
...
const selectedEventChips = computed(() => {
  const nameMap = Object.fromEntries(
    props.dbEvents.map(ev => [ev.eventId, `${ev.eventId} - ${ev.name || ev.eventId}`])
  )
  return props.compareEventIds.map(eventId => ({
    eventId,
    label:
      eventId === props.currentEventId && props.currentEventName
...
const summaryRows = computed(() => {
  ...
  const minX50 = rows.reduce(
    (best, row) => (row.fragmentX50 < best.fragmentX50 ? row : best),
    rows[0]
  )
  const maxThrow = rows.reduce(
    (best, row) => (row.throwDistanceMax > best.throwDistanceMax ? row : best),
    rows[0]
  )
  const minVibration = rows.reduce(
    (best, row) => (row.vibrationPeak < best.vibrationPeak ? row : best),
    rows[0]
  )
  const maxCrater = rows.reduce(
    (best, row) => (row.craterDepth > best.craterDepth ? row : best),
    rows[0]
  )
  ...
  return [
    ...
    },
    {
      title: '飞散最强',
      name: maxThrow.name,
      value: maxThrow.throwDistanceMax,
      unit: 'm',
      note: '抛距最高',
      tone: 'warn'
    },
    {
      title: '扰动最保守',
      name: minVibration.name,
      value: minVibration.vibrationPeak,
      unit: 'cm/s',
      note: '振动最低',
      tone: 'ok'
    },
...
function removeEvent(eventId) {
  emit(
    'update:compare-event-ids',
    props.compareEventIds.filter(id => id !== eventId)
  )
}
...
.cmp-table-row {
  ...
  grid-template-columns:
    minmax(0, 1fr) minmax(0, 0.55fr) minmax(0, 0.55fr) minmax(0, 0.6fr) minmax(0, 0.7fr)
