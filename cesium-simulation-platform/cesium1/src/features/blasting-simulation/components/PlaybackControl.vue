<template>
  <div class="panel-section">
    ...
    <div v-if="dataset" class="controls-row mt-3">
      ...
      <button
        class="compact-action-btn"
        :class="{ primary: abLoop.enabled }"
        :disabled="!dataset"
        :title="'在当前帧标记 A/B 点（当前帧 ' + (currentFrame + 1) + '）'"
        @click="$emit('mark-ab-loop')"
      >
      ...
      <button
        ...
        class="compact-action-btn"
        :class="{ primary: abLoop.enabled }"
        @click="$emit('toggle-ab-loop')"
      >
        AB 循环：{{ abLoop.enabled ? '开' : '关' }}
      </button>
    </div>
    <div v-if="abLoop.a != null || abLoop.b != null" class="hint-text">
      A 点：{{ abLoop.a != null ? '帧 ' + (abLoop.a + 1) : '未设置' }} | B 点：{{
        abLoop.b != null ? '帧 ' + (abLoop.b + 1) : '未设置'
      }}
    </div>
    ...
    <div v-if="threeStats" class="mt-3">
      ...
      <div class="stat-item">
        <span>目标碎片数</span><span class="stat-value">{{ threeStats.fragmentCountTarget }}</span>
      </div>
      <div class="stat-item">
        <span>生成碎片数</span
        ><span class="stat-value">{{ threeStats.fragmentCountGenerated }}</span>
      </div>
      <div class="stat-item">
        <span>渲染碎片数</span
        ><span class="stat-value">{{ threeStats.fragmentCountRendered }}</span>
      </div>
      <div class="stat-item">
      ...
      <div class="stat-item">
        ...
        ><span class="stat-value">{{
          threeStats.kcoSourceMode === 'result' ? '结果驱动' : '设计驱动'
        }}</span>
      </div>
      <div v-if="threeStats.fragmentMassTargetKg" class="stat-item">
        <span>目标可见质量</span
        ><span class="stat-value">{{ threeStats.fragmentMassTargetKg.toFixed(0) }} kg</span>
      </div>
      <div v-if="threeStats.fragmentMassGeneratedKg" class="stat-item">
        <span>生成可见质量</span
        ><span class="stat-value">{{ threeStats.fragmentMassGeneratedKg.toFixed(0) }} kg</span>
      </div>
      <div v-if="threeStats.throwDistancePredictedAvg" class="stat-item">
        <span>预测平均抛距</span
        ><span class="stat-value">
          {{ threeStats.throwDistancePredictedAvg.toFixed(1) }} m
          <template v-if="threeStats.throwDistanceTargetAvg">
            / 目标 {{ threeStats.throwDistanceTargetAvg.toFixed(1) }} m
          </template>
        </span>
      </div>
      <div v-if="threeStats.throwDistancePredictedMax" class="stat-item">
        <span>预测最大抛距</span
        ><span class="stat-value">
          {{ threeStats.throwDistancePredictedMax.toFixed(1) }} m
          <template v-if="threeStats.throwDistanceTargetMax">
            / 目标 {{ threeStats.throwDistanceTargetMax.toFixed(1) }} m
          </template>
        </span>
      </div>
    </div>
