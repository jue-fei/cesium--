...
function buildBuilder() {
  ...
  const config = {
    ...
    designParams: { holeDepth: 2.5 }, craterSeed: 7, surfacePolish: false
  }
...
function edgeReport(geo, q = 4) {
  ...
  for (let t = 0; t < triCount; t++) {
    ...
    for (const [p,q2] of [[a,b],[b,c],[c,a]]) {
      ...
      if (!samp.has(key)) samp.set(key, [pa[p*3], pa[p*3+1], pa[p*3+2], pa[q2*3], pa[q2*3+1], pa[q2*3+2]])
    }
  }
  let open=0, over=0
  const openE=[]
  for (const [k,n] of use) {
    if (n===1){open++; openE.push(samp.get(k)); const z=+samp.get(k)[2].toFixed(2); zhist.set(z,(zhist.get(z)||0)+1)}
    if (n>2) over++
  }
...
describe('POST geometry hole/gap root cause', () => {
  it('uncut POST watertight + open edges grouped by z', () => {
    ...
    console.log('z-hist (open edge count per z):', JSON.stringify([...r.zhist].sort((a,b)=>a[0]-b[0])))
  ...
  it('around z=roundDepth: does ring wall inner rim meet center disc?', () => {
    ...
    const r = edgeReport(post, 4)
    const atPlane = r.openE.filter(e => Math.abs(e[2]-2.125)<0.01 && Math.abs(e[5]-2.125)<0.01)
