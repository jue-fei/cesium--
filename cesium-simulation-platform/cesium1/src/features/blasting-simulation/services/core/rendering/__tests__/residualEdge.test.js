...
function edgeReport(geo, q = 6) {
  ...
  const pa = px.array
  const idx = geo.index; const ta = idx ? idx.array : null
  ...
  for (let t=0;t<tc;t++){
    const a=ta?ta[t*3]:t*3,b=ta?ta[t*3+1]:t*3+1,c=ta?ta[t*3+2]:t*3+2
    for(const [p,q] of [[a,b],[b,c],[c,a]]){const k=ek(vk(p),vk(q)); use.set(k,(use.get(k)||0)+1); if(!samp.has(k))samp.set(k,[pa[p*3],pa[p*3+1],pa[p*3+2],pa[q*3],pa[q*3+1],pa[q*3+2]])}
  }
  let open=0; const oe=[]
  for(const [k,n] of use){ if(n===1){open++; oe.push(samp.get(k))} }
  return {open, oe}
}
  ...
  for (const axis of [0,1]) {
    it(`axis=${axis} residual open edge coords`, () => {
      ...
      for (const frac of [0.25, 0.6, 0.9]) {
        const pos = bmin.getComponent(axis) + (bmax.getComponent(axis)-bmin.getComponent(axis))*frac
        ...
        console.log(`axis=${axis} frac=${frac} pos=${pos.toFixed(2)} open=${r.open} edges=${JSON.stringify(r.oe)}`)
      }
    })
  }
})