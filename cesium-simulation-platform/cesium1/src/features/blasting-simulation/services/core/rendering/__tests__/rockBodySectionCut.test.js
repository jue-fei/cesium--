...
function edgeUseCount(geo) {
  ...
  for (let t = 0; t < triCount; t++) {
    ...
    for (const [p,q] of [[0,1],[1,2],[2,0]]) {
      ...
      if (!edgeSample.has(key)) edgeSample.set(key, { a: k3[p].split('|').map(Number), b: k3[q].split('|').map(Number) })
    }
  }
  let open = 0, over = 0
  const openPts = []
  for (const [k, n] of edgeUse) {
    if (n === 1) { open++; const s = edgeSample.get(k); openPts.push(s) }
    if (n > 2) over++
  }
  ...
  for (const axis of [0, 1, 2]) {
    ...
    it(`${name}沿轴${axis} 做多处剖切，检查剖切结果水密性`, () => {
      ...
      for (const frac of [0.25, 0.5, 0.75, 0.1]) {
        ...
        console.log(`axis=${axis} frac=${frac} pos=${pos.toFixed(2)} 剖切后: openEdges=${r.open} overUsed=${r.over} tris=${r.triCount}`)
