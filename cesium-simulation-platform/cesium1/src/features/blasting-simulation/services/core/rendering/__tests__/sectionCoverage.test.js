...
function triFaces(geo) {
  ...
  for (let t=0;t<tc;t++){
    ...
    out.push([[pa[a*3],pa[a*3+1],pa[a*3+2]],[pa[b*3],pa[b*3+1],pa[b*3+2]],[pa[c*3],pa[c*3+1],pa[c*3+2]]])
  }
  return out
}
...
describe('POST section is visually solid (no big hole)', () => {
  it('axis0 x=-36: lower region (z<roundDepth) is covered by fill triangles', () => {
    ...
    const lo = faces.filter(f => f.every(p => Math.abs(p[0]-pos) < 0.2) && (f[0][2]+f[1][2]+f[2][2])/3 < rd - 0.05)
    const hi = faces.filter(f => f.every(p => Math.abs(p[0]-pos) < 0.2) && (f[0][2]+f[1][2]+f[2][2])/3 > rd + 0.05)
