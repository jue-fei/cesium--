...
function report(label, geo, rd, tol = 0.05) {
  ...
  for (let t = 0; t < tc; t++) {
    ...
    if (Math.abs(za - rd) < tol && Math.abs(zb - rd) < tol && Math.abs(zc - rd) < tol) {
      ...
      let inTunnel = false
      if (cy <= Hw) {
        inTunnel = Math.abs(cx) <= halfW
      } else {
        const dy = cy - Hw
        inTunnel = cx * cx + dy * dy <= R * R
      }
  ...
  console.log(
    `[${label}] rd=${rd} trisOnPlane(t${tunnelNegZ}+t${tunnelPosZ} / a${annulusNegZ}+a${annulusPosZ})  tunnel: -z=${tunnelNegZ} +z=${tunnelPosZ}; annulus: -z=${annulusNegZ} +z=${annulusPosZ}`
  )
}
...
describe('face plane wind / overlap diagnostics', () => {
  it('replicate pieces independently', () => {
    ...
    // 主线 SceneBuilder 已无 _createRockShape（更名为 _buildInflatedRockShape，签名多了 rockThickness）
    const rockShape = sb._buildInflatedRockShape
      ? sb._buildInflatedRockShape(W, Hw, R, ctx.rockThickness || 5)
      : null
    ...
    let core1 = new THREE.ExtrudeGeometry(rockShape, { ...extrudeOpts, depth: 40 })
