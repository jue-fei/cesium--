...
describe('debug slit axis0', () => {
  it('dump open edges for x=0 (through tunnel) loop structure', () => {
    ...
    const bmin = post.boundingBox.min, bmax = post.boundingBox.max
    ...
    const ptk = (x,y,z)=>`${Math.round(x*1000)}|${Math.round(y*1000)}|${Math.round(z*1000)}`
    const adj = new Map()
    const link=(a,b)=>{if(!adj.has(a))adj.set(a,[]);if(!adj.has(b))adj.set(b,[]);adj.get(a).push(b);adj.get(b).push(a)}
    const pts = new Map()
    for (const e of open){
      const a=ptk(e[0],e[1],e[2]), b=ptk(e[3],e[4],e[5])
      link(a,b); if(!pts.has(a))pts.set(a,[e[0],e[1],e[2]]); if(!pts.has(b))pts.set(b,[e[3],e[4],e[5]])
    }
    const visited=new Set(); const loops=[]
    for(const s of adj.keys()){
      if(visited.has(s))continue
      let cur=s, prev=null, ring=[]; const guard=new Set()
      while(cur!=null && !guard.has(cur)){
        guard.add(cur); visited.add(cur); ring.push(cur)
    ...
    loops.forEach((r,i)=>{
      const zs=r.map(k=>pts.get(k)[2]); const ys=r.map(k=>pts.get(k)[1])
      const zr=[Math.min(...zs),Math.max(...zs)], yr=[Math.min(...ys),Math.max(...ys)]
      console.log(`loop${i}: len ${r.length} yRange[${yr[0].toFixed(1)},${yr[1].toFixed(1)}] zRange[${zr[0].toFixed(2)},${zr[1].toFixed(2)}]`)
    })
  })
})