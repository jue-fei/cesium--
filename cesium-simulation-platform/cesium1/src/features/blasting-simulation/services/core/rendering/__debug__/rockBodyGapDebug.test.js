...
describe('POST 几何水密性诊断', () => {
  it('统计开放边界边（单面边=水密性失败）与重叠边', () => {
    ...
    const vk = i =>
      `${pa[i * 3].toFixed(4)}|${pa[i * 3 + 1].toFixed(4)}|${pa[i * 3 + 2].toFixed(4)}`
    ...
    for (let t = 0; t < triCount; t++) {
      ...
      for (const [p, q] of pairs) {
        ...
        if (!edgeSamples.has(key))
          edgeSamples.set(key, { a: k3[p].split('|').map(Number), b: k3[q].split('|').map(Number) })
      }
    }
    const open = []
    const over = []
    ...
    const zAt = k => {
      const s = edgeSamples.get(k)
      return (s.a[2] + s.b[2]) / 2
    }
    const yAt = k => {
      const s = edgeSamples.get(k)
      return (s.a[1] + s.b[1]) / 2
    }
    ...
    const sample = open.slice(0, 12).map(k => {
      const s = edgeSamples.get(k)
      return `z~${zAt(k).toFixed(2)} y~${yAt(k).toFixed(2)} @(${s.a.map(x => x.toFixed(1)).join(',')})-(${s.b.map(x => x.toFixed(1)).join(',')})`
    })
    ...
    for (let t = 0; t < triCount; t++) {
      ...
      if (vx.every(i => Math.abs(posA[i * 3 + 2] - rd) < 0.05)) {
        ...
        if (Math.abs(cx) < 2.5) {
          ...
          const e1x = posA[vx[1] * 3] - posA[vx[0] * 3],
            e1y = posA[vx[1] * 3 + 1] - posA[vx[0] * 3 + 1],
            e1z = posA[vx[1] * 3 + 2] - posA[vx[0] * 3 + 2]
          const e2x = posA[vx[2] * 3] - posA[vx[0] * 3],
            e2y = posA[vx[2] * 3 + 1] - posA[vx[0] * 3 + 1],
            e2z = posA[vx[2] * 3 + 2] - posA[vx[0] * 3 + 2]
    ...
    console.log(
      `新掌子面中心区三角形: count=${countOnFace} 法线-z(面朝隧道口)占比=${countOnFace ? ((negZ / countOnFace) * 100).toFixed(1) : '-'}%`
    )
    // 打印平面三角的分布：按 (x,y) 归类 +z 与 -z，帮助判断来源（环背盖 or 中心盘）
    const buckets = {}
    for (let t = 0; t < triCount; t++) {
      const vx = [
        triArr ? triArr[t * 3] : t * 3,
        triArr ? triArr[t * 3 + 1] : t * 3 + 1,
        triArr ? triArr[t * 3 + 2] : t * 3 + 2
      ]
    ...
    const stable = Object.entries(buckets)
      .map(([k, v]) => `${k}(-${v.n}/+${v.p})`)
      .join('  ')
    ...
    for (let t = 0; t < pidx.length; t += 3) {
      const a = pidx[t],
        b = pidx[t + 1],
        c = pidx[t + 2]
      const e1x = pp[b * 3] - pp[a * 3],
        e1y = pp[b * 3 + 1] - pp[a * 3 + 1]
      const e2x = pp[c * 3] - pp[a * 3],
        e2y = pp[c * 3 + 1] - pp[a * 3 + 1]
      const nz = e1x * e2y - e1y * e2x
      if (nz < 0) pNeg++
      else pPos++
    }
    ...
    try {
      ...
      for (let t = 0; t < pi.length; t += 3) {
        const a = pi[t],
          b = pi[t + 1],
          c = pi[t + 2]
        const e1x = pp2[b * 3] - pp2[a * 3],
          e1y = pp2[b * 3 + 1] - pp2[a * 3 + 1]
        const e2x = pp2[c * 3] - pp2[a * 3],
          e2y = pp2[c * 3 + 1] - pp2[a * 3 + 1]
        if (e1x * e2y - e1y * e2x < 0) z2++
        else o2++
      }
      console.log(`probe disc crease后  法线-z=${z2} 法线+z=${o2}`)
    } catch (e) {
      console.log('crease import fail', e.message)
    }
  })
})
