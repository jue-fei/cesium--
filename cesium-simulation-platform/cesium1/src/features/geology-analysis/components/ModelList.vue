<template>
  <div
    ...
    <div class="flex justify-between items-center mb-3">
      <span
        class="text-sm font-semibold text-blue-100 flex items-center gap-2 before:content-[''] before:w-1 before:h-3 before:bg-blue-500 before:rounded-sm">模型管理</span>
      <button v-if="!isConfigPanelOpen"
        class="p-1 rounded bg-transparent text-gray-400 hover:bg-white/10 hover:text-white transition-colors" title="配置"
    ...
    <transition enter-active-class="transition-all duration-300 ease-out"
      enter-from-class="max-h-0 opacity-0 overflow-hidden" enter-to-class="max-h-[200px] opacity-100"
      leave-active-class="transition-all duration-300 ease-in" leave-from-class="max-h-[200px] opacity-100"
      ...
      <div v-if="isConfigPanelOpen" class="mb-3 bg-black/20 rounded p-3 border border-white/5">
        <div class="mb-2">
          <div class="flex justify-between items-center mb-1 text-xs text-gray-400">
            <span>配置文件</span>
            <button class="text-blue-400 hover:text-blue-300 hover:underline" @click="toggleConfigPanel">
              收起
            </button>
          </div>
          <select v-model="currentConfigFileLocal" :disabled="undergroundViewEnabled || switching"
        ...
        <div class="flex gap-2">
          <button :disabled="undergroundViewEnabled || switching"
    ...
    <div class="mb-3">
      ...
      <div>
        <input v-model.number="globalOpacityLocal" type="range" min="0" max="100" step="1"
          ...
          @input="handleGlobalOpacityChange" @change="handleGlobalOpacityChange" />
      </div>
    </div>

    <!-- 模型列表 -->
    <div class="max-h-[200px] overflow-y-auto border border-white/10 rounded-md bg-black/20 custom-scrollbar">
      <div v-for="model in modelList" :key="model.id"
        class="p-2 border-b border-white/5 transition-colors last:border-0"
        :class="{ 'bg-white/5': hoveredModelId === model.id }" @mouseenter="hoveredModelId = model.id"
        ...
        <div class="flex justify-between items-center">
          <label class="flex items-center gap-2 cursor-pointer group">
            <div class="relative flex items-center">
              ...
              <svg class="absolute w-3.5 h-3.5 pointer-events-none hidden peer-checked:block text-white"
            ...
            <div class="flex flex-col gap-0.5">
              <span class="text-xs text-gray-300 group-hover:text-white transition-colors">{{ model.name
              }}<span v-if="model._dbLinked" class="ml-1 text-[10px] text-blue-400" title="数据来源：数据库">◈DB</span></span>
              <span class="text-[10px] text-gray-500 truncate max-w-[160px]">{{
                model.geologyProperties?.['地质类型'] || model.type || ''
                }}</span>
            </div>
          </label>
          <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity"
            :class="{ 'opacity-100': hoveredModelId === model.id }">
            <button class="p-1 rounded hover:bg-white/10 text-gray-400 hover:text-yellow-300 transition-colors"
              title="高亮" @click="highlightModel(model)">
              🔆
            </button>
            <button class="p-1 rounded hover:bg-white/10 text-gray-400 hover:text-blue-300 transition-colors" title="属性"
        ...
        <!-- 单个模型透明度 -->
        <div v-show="hoveredModelId === model.id || model.opacity > 0" class="mt-1.5 px-1">
          <input v-model.number="model.opacity" type="range" min="0" max="100" step="1"
            class="w-full h-1 bg-white/20 rounded-lg appearance-none cursor-pointer hover:bg-blue-500/50 accent-blue-500 transition-colors"
            @input="updateModelOpacity(model)" @change="updateModelOpacity(model)" />
        </div>
      </div>
    </div>
...
const globalOpacityLocal = ref(0)
