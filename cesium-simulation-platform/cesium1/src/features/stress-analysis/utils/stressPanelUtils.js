...
function getWebglInfo() {
  ...
  if (!gl) return { webgl: false, webgl2: false, renderer: '', vendor: '' }
...
export function measureEvaluation(durationMs) {
  return new Promise(resolve => {
    ...
    const tick = now => {
      ...
      const avgFps = frames / Math.max(0.001, seconds)
      intervals.sort((a, b) => a - b)
      const p95FrameMs =
        intervals.length > 0 ? intervals[Math.floor(0.95 * (intervals.length - 1))] : 0
      ...
      resolve({
        ...
        环境: {
          ...
          // @ts-expect-error deviceMemory is experimental API not in TS types
          deviceMemory: navigator.deviceMemory || null,
          webglVendor: webgl.vendor,
          webglRenderer: webgl.renderer,
          // @ts-expect-error VERSION may not be in Cesium type definitions
          cesiumBuild: Cesium.VERSION || ''
        }
      })
    }
    requestAnimationFrame(tick)
  }).then(raw => {
    const performanceInfo = raw['性能']
    const compat = raw['兼容性']
    return {
      metadata: {
        生成时间: raw['时间'],
        模块: '应力分析渲染评估'
      ...
      performance: {
        durationSec: performanceInfo['采样秒数'],
        avgFps: performanceInfo['平均FPS'],
        p95FrameMs: performanceInfo['P95帧耗时ms']
      },
