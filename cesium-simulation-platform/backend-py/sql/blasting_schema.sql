...
-- ─── 删除旧的关系型表（重建）──────────────────────────────
DROP TABLE IF EXISTS `blasting_runtime_stats`;
...
CREATE TABLE `blasting_result` (
  ...
  -- 碎块（KCO 输出）
  `fragment_count` INT COMMENT '碎片总数 (deprecated: 使用 fragment_count_generated)',
  -- 碎块数量细分（可追溯）
  `fragment_count_target` INT COMMENT '目标碎石数（KCO+体积驱动计算）',
  `fragment_count_generated` INT COMMENT '算法生成碎石数',
  `fragment_count_rendered` INT COMMENT '实际渲染碎石数（受上限裁剪）',
  -- 碎块质量与体积
  `fragment_mass_target_kg` DOUBLE COMMENT '目标总质量(kg)',
  `fragment_mass_generated_kg` DOUBLE COMMENT '生成总质量(kg)',
  `fragment_volume_target` DOUBLE COMMENT '目标爆破方量(m³)',
  `fragment_volume_generated` DOUBLE COMMENT '生成方量(m³)',
  -- 分布直方图（JSON）
  `fragment_histogram_json` JSON COMMENT '块度分布直方图 [{min,max,count,pct}]',
  `velocity_histogram_json` JSON COMMENT '速度分布直方图 [{min,max,count,pct}]',
  `throw_distance_histogram_json` JSON COMMENT '抛距分布直方图 [{min,max,count,pct}]',
  -- 渲染尺寸模式
  `render_scale_mode` VARCHAR(16) DEFAULT 'min_pixel' COMMENT '渲染尺寸模式: min_pixel/absolute',
  `render_scale_bias` DOUBLE DEFAULT 0 COMMENT '渲染尺寸偏移(m)',
...
-- ============================================================
-- 4.5 blasting_runtime_stats 运行时统计表（与事件 1:N，每次 replay 生成一行）
-- 记录历次模拟的随机种子、算法版本、参数快照、统计快照，支持历史复现
-- ============================================================
CREATE TABLE `blasting_runtime_stats` (
  `id` INT AUTO_INCREMENT PRIMARY KEY,
  `event_id` VARCHAR(32) NOT NULL,
  `random_seed` INT COMMENT '随机数种子（保证回放可复现）',
  `algorithm_version` VARCHAR(32) COMMENT '算法版本号',
  `params_snapshot` JSON COMMENT '核心输入参数快照（chargeKg, x50, xmax 等）',
  `stats_snapshot` JSON COMMENT '运行时统计快照（fragmentCountGenerated/Rendered, mass, histograms 等）',
  `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
